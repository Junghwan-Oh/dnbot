...
5. 현재 즉시 실행 baseline lane은 `Nado 1DEX ETH-SOL pair` 이다
6. 최종 target은 `1DEX + subaccount` same-asset hedge 이다
7. 단, subaccount / leverage / health truth가 검증되기 전까지는 feasibility lane이다
8. venue rollout 우선순위는 `Nado -> GRVT -> Variational` 이다
9. `Variational` 은 API 조회 중심 + 컴포넌트 조작 주문 구조 때문에 최종 확장 lane으로 본다

## 현재 운영 doctrine

- `PnL > 0` 는 최종 목표지만, donor 채택의 1차 기준은 **매매 인프라 안정성** 이다
- 최우선 평가 항목은 아래와 같다
  - one-leg fill 방지
  - residual position 방지
  - flat-close determinism
  - 진입 전 기존 포지션 / 기존 주문 상태 확인
  - 청산 전후 position truth 일치
  - orphaned position 방지
- market order logic는 기본적으로 메인라인 후보에서 불채택한다
- 기본 execution doctrine은 `limit-first` 이며, 대략 `30분 limit-window` 안에서 더 유리한 체결을 우선 노린다
- 반대 방향 급변 리스크는 stop-loss로 방어하고, trailing stop / entry-time stop-loss 는 후속 PnL 개선 옵션으로 유지한다
- `alternate` 는 현재 임시 baseline 전략일 뿐, 고정 doctrine이 아니다
- spread 기반 venue-role assignment 와 `alternate` 는 상충 가능성이 있으므로, 후속 benchmark / WS capability / execution quality / PnL 테스트로 최적화한다
